...
const sketch = ({ width, height }) => {
  ...
  createGrid(15, 15).forEach(([u, v]) => {
    const point = [
      math.lerp(margin, width - margin, u),
      math.lerp(margin, height - margin, v)
    ];
    const randomAngle = random.noise2D(u, v, 0.2) * Math.PI * 2;
    const leafLines = createLeafLines(point, margin * 2, randomAngle);
...
function createLeafLines(leafStart = [0, 0], length = 0, angle = 0) {
  ...
  array(spinePointCount).forEach((_, i) => {
    ...
    let angleShift = Math.abs(
      Math.PI * 2 * leafRandom.noise2D(lastPoint[0], lastPoint[1], 1, 0.1)
    );
